...
from byol.modeling.model import BootstrapYourOwnLatent
from byol.config import encoder, projector, predictor, loss_function, NUM_EPOCHS, transforms, BATCH_SIZE, SHUFFLE, TAU, PATH_OF_THE_SAVED_MODEL_PARAMETERS
import torch.optim as optim

model = BootstrapYourOwnLatent(encoder, projector, predictor, loss_function, TAU)
...
train_dataloader = DataLoader(
            dataset=transformed_dataset, batch_size=BATCH_SIZE, shuffle=SHUFFLE
        )
...
for epoch in range(NUM_EPOCHS):
    for index, (view1, view2, label) in enumerate(train_dataloader):
        ...
        if index % 100 == 0:
            print(f'Epoch [{epoch + 1}/{NUM_EPOCHS}], Iter [{index + 1}/{len(train_dataloader)}], Loss: {loss.item():.4f}')
